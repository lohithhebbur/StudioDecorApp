...
(function () {
  ...
  ddlCustomer.addEventListener("change", () => {
    ...
    if (gstinField && !gstinField.value.trim()) {
      const picked = customers.find(c => c.id === ddlCustomer.value);
      if (picked && picked.gstin) gstinField.value = picked.gstin;
    }
  });
  ...
  function openNewQuotation() {
    ...
    document.getElementById("quoFirmGstin").value = readFirmDetails().firm.gstin || "";
    document.getElementById("quoCustomerGstin").value = "";
    updatePreview();
  ...
  function openEditQuotation(id) {
    ...
    document.getElementById("quoFirmGstin").value = q.firmGstin || readFirmDetails().firm.gstin || "";
    document.getElementById("quoCustomerGstin").value = q.customerGstin || "";
  ...
  function renderInvoiceToggle(q) {
    ...
    const editRow = document.getElementById("quoInvoiceNumberEditRow");
    const prefixSpan = document.getElementById("quoInvoiceNumberPrefix");
    const suffixInput = document.getElementById("quoInvoiceNumberSuffix");
    checkbox.checked = !!q.isInvoice;
    if (q.isInvoice && q.invoiceNumber) {
      const parts = q.invoiceNumber.split("/");
      const suffix = parts.pop();
      prefixSpan.textContent = parts.join("/") + "/";
  ...
  document.getElementById("quoInvoiceNumberSuffix").oninput = e => {
    const q = quotations.find(x => x.id === editingId);
    if (!q) return;
    const digits = e.target.value.replace(/\D/g, "").slice(0, 3).padStart(3, "0");
    const prefix = document.getElementById("quoInvoiceNumberPrefix").textContent;
    q.invoiceNumber = `${prefix}${digits}`;
    persist();
  };
  ...
  function saveQuotation() {
    ...
    const record = {
      ...
      customerAddress: resolvedCustomer ? (resolvedCustomer.address || resolvedCustomer.locality || "") : "",
      customerGstin: document.getElementById("quoCustomerGstin").value.trim() || (resolvedCustomer ? (resolvedCustomer.gstin || "") : ""),
      firmGstin: document.getElementById("quoFirmGstin").value.trim(),
  ...
  function printQuotation(id) {
    ...
    const firmDetailRows = [
      ...
      [(q.firmGstin || firm.gstin) ? `GSTIN: ${q.firmGstin || firm.gstin}` : "", `<rect x="3" y="4" width="18" height="16" rx="2"/><path d="M7 8h10M7 12h10M7 16h6"/>`]
    ];
